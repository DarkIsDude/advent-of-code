...
func main() {
	...
	for maxPosition != minPosition {
		...
		fmt.Printf("We are testing the position %d (min %d, max %d)\n", positionToTest, minPosition, maxPosition)
...
func calculateFuelNeedTo(crabsPosition []int, desiredPosition int) int {
	...
	for _, crabPosition := range crabsPosition {
		localFuelNeeded := math.Abs(float64(desiredPosition - crabPosition))
