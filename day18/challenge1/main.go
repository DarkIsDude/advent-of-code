...
func main() {
	snailfishs := readFile()
	snailfishSum := reduce(snailfishs[0])

	for _, snailfish := range snailfishs[1:] {
		snailfishSum = []interface{}{snailfishSum, reduce(snailfish)}
		snailfishSum = reduce(snailfishSum)
	}

	var snailfishAsArray []interface{} = snailfishSum.([]interface{})
	fmt.Println(magnitude(snailfishAsArray))
}
