...
func isValidPath(path []string) bool {
	var onlyOnePassCaveAllowed []string

	for _, cave := range path {
		if unicode.IsLower([]rune(cave)[0]) {
			onlyOnePassCaveAllowed = append(onlyOnePassCaveAllowed, cave)
		}
	}

	for posA, caveA := range onlyOnePassCaveAllowed {
		for posB, caveB := range onlyOnePassCaveAllowed {
