...
func main() {
	...
	for atLeastOneFound {
		...
		for i := 0; i < len(scanners); i++ {
			...
			ok, beacons := rotateScanner2AndCompare(scanners[0].beacons, scanners[i].beacons)
			if ok {
				atLeastOneFound = true
				scanners[i].matched = true

				fmt.Printf(
					"New scanner position detected %d\n",
					scanners[i].name,
				)
...
func rotateScanner2AndCompare(beacons1 []Beacon, beacons2 []Beacon) (bool, []Beacon) {
			...
			for _, ySign := range []int{1, -1} {
				for _, zSign := range []int{1, -1} {
					...
					if ok {
						...
						return true, moveBeacons(diffX, diffY, diffZ, newBeacons2, true)
					}
				}
			}
		}
