...
func main() {
	...
	for scanner.Scan() {
		...
		guessValue := guessPreviousValue(values)
...
func guessPreviousValue(values []int) int {
	fmt.Println(values)
	...
	nextValueOfDifferences := guessPreviousValue(differences)

	return values[0] - nextValueOfDifferences
}
